...
void Entity381::Init(){
	...
	cameraNode = sceneNode->createChildSceneNode();
	cameraNode->yaw(Ogre::Radian(Ogre::Degree(-90)));
	if(identity == SpeedBoatType || identity == BansheeType || identity == AlienType)
	{
	    cameraNode->translate(0, 20, 0);
	}
	else
	{
	    cameraNode->translate(-200, 75, 0);
	}
	sceneNode->attachObject(ogreEntity);
}
